...
for t in target:
    ...
    for idx in range(loop):
        ...
        for each_event in events:
            ...
            node = tagger.parseToNode(each_event['title'])
            target_parts_of_speech = ('固有名詞', )
            while node:
                if node.feature.split(',')[1] in target_parts_of_speech:
                    #print(node.surface)
                    text[year + month].append(node.surface)
                node = node.next
